...
class JDocumentRendererFeedRss extends JDocumentRenderer
{
	...
	public function render($name = '', $params = null, $content = null)
	{
		...
		$syndicationURL = JRoute::_('&format=custfeed&type=rss');
		...
		for ($i = 0, $count = count($data->items); $i < $count; $i++)
		{
			...
			if ($data->items[$i]->image != null)
			{
				...
				{
					$imagelink = str_replace(' ', '%20', $url . '/' . $imagelink);
				}
				$feed .= "			<image>\n";
				$feed .= "				<url>" . $image->url . "</url>\n";
				$feed .= "				<title>" . htmlspecialchars($image->title, ENT_COMPAT, 'UTF-8') . "</title>\n";
				$feed .= "				<link>" . str_replace(' ', '%20', $imagelink) . "</link>\n";

				if ($image->width != "")
				{
					$feed .= "				<width>" . $image->width . "</width>\n";
				}

				if ($image->height != "")
				{
					$feed .= "				<height>" . $image->height . "</height>\n";
				}

				if ($image->description != "")
				{
					$feed .= "				<description><![CDATA[" . $image->description . "]]></description>\n";
				}
